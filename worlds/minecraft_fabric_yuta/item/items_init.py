...
from worlds.minecraft_fabric_yuta.item.items_helper import add_item_to_pool, add_items_to_pool, get_progression_bl_items, \
    create_item, get_junk_items, get_item, add_optional_item, get_blank_filler
from worlds.minecraft_fabric_yuta.item.item_manager import ProcessedMinecraftItem
...
def get_filler(world: FabricMinecraftWorld, total_items: int):
    ...
    trap_weights += add_trap_weight("Lightning Trap", world.options.lightning_trap_weight)
    trap_weights += add_trap_weight("Item Drop Trap", world.options.item_drop_trap_weight)
    trap_weights += add_trap_weight("Nighttime Trap", world.options.nighttime_trap_weight)
    trap_weights += add_trap_weight("Box Trap", world.options.box_trap_weight)
    trap_weights += add_trap_weight("Shuffle Trap", world.options.shuffle_trap_weight)
