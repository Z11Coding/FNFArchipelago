...
class yutautil_APYaml:
    ...
    def __init__(self,yamlContent):
        ...
        if self.settings is not None and Reflect.field(self.settings, "songList") is None:
            self._findSongListInSettings()

    def _findSongListInSettings(self):
        """Check every field in settings to see if any contains a songList"""
        if self.settings is None:
            return
            
        # Get all field names from settings
        try:
            if hasattr(self.settings, '_hx_fields'):
                field_names = self.settings._hx_fields
            else:
                # Fallback: try common attribute inspection
                field_names = [attr for attr in dir(self.settings) if not attr.startswith('_')]
            
            # Check each field for a songList
            for field_name in field_names:
                field_value = Reflect.field(self.settings, field_name)
                if field_value is not None:
                    # Check if this field has a songList
                    songList = Reflect.field(field_value, "songList")
                    if songList is not None:
                        # Set it directly on settings
                        Reflect.setField(self.settings, "songList", songList)
                        if self.settings.songList is None:
                            self.settings.songList = songList
                        return
        except:
